import React from 'react';

const BASE_STAGES = [
  { step: 1, label: 'Core Audience', code: 'AUDIENCE' },
  { step: 2, label: 'Distinct Edge', code: 'DIFFERENTIATION' },
  { step: 3, label: 'Voice & Edge', code: 'BRAND EDGE' }
];

export default function ProgressStepper({ currentRound = 1, readyForSynthesis = false, stageLabel = '' }) {
  const isDeepening = currentRound > 3;

  const stagesToRender = isDeepening
    ? [
        ...BASE_STAGES,
        { step: currentRound, label: stageLabel || 'Deepening Moat', code: `ROUND ${currentRound}` }
      ]
    : BASE_STAGES;

  const colsClass = isDeepening ? 'grid-cols-4' : 'grid-cols-3';

  return (
    <div className="w-full max-w-xl mx-auto px-4">
      {/* Centered Segmented Step Indicators */}
      <div className={`grid ${colsClass} gap-2 sm:gap-3 mb-2`}>
        {stagesToRender.map((stage) => {
          const isDone = currentRound > stage.step;
          const isActive = currentRound === stage.step;

          return (
            <div key={stage.step} className="flex flex-col gap-1.5">
              {/* Segmented bar indicator */}
              <div 
                className={`h-1 rounded-full transition-all duration-300 ${
                  isDone 
                    ? 'bg-black' 
                    : isActive 
                      ? 'bg-black' 
                      : 'bg-[#dbd7cd]'
                }`}
              />
              <div className="flex items-center justify-between">
                <span className={`text-[10px] uppercase tracking-wider font-sans truncate transition-colors ${
                  isActive ? 'text-black font-medium' : isDone ? 'text-stone-700' : 'text-stone-400'
                }`}>
                  0{stage.step} &bull; {stage.code}
                </span>
              </div>
            </div>
          );
        })}
      </div>
    </div>
  );
}
