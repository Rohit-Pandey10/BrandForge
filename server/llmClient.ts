 ...
 */
export async function generateStructuredJson<T = Record<string, unknown>>(options: GenerateLlmOptions): Promise<T> {
  ...
  if (effectiveGroqKey && (preferredProvider === 'groq' || !effectiveGeminiKey)) {
    ...
    for (let i = 0; i < GROQ_CANDIDATE_MODELS.length; i++) {
      ...
      try {
        ...
        console.warn(`[LLM Client] Groq model [${model}] failed (${error.message}). Trying fallback...`);
  ...
  if (effectiveGeminiKey) {
    ...
    for (let i = 0; i < GEMINI_CANDIDATE_MODELS.length; i++) {
      ...
      try {
        ...
        const error = err as Error;
        console.warn(`[LLM Client] Gemini model [${model}] failed (${error.message}). Trying fallback...`);
      }
    }
  }
