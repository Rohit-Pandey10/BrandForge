...
import { PRESET_PITCHES, generateGenericMockQuestion, generateGenericMockBrandKit } from './mockData';
import { DOMAINS, classifyDomain, isFamilyIntent, extractPitchMetadata } from './domainConfig';
import { generateStructuredJson, isLlmConfigured } from './llmClient';
import { BrandKit, QuestionResponse } from '../src/types';
...
export async function getNextQuestion({
  ...
  groqApiKey = null,
  useMock = false
}: NextQuestionParams): Promise<QuestionResponse> {
  ...
  // If user explicitly asks for mock or no LLM provider is configured
  if (useMock || !hasKey) {
...
Generate the curated Socratic question for Round ${currentRound}. Provide exactly 3 suggested answer pills with distinct conviction choices specifically suited to this concept.`;
  ...
  try {
    const parsed = await generateStructuredJson<QuestionResponse>({
      systemInstruction,
    ...
    console.warn(`[GeminiService] Live LLM calls failed, falling back to domain-aware mock:`, error.message);
    if (matchedPreset && matchedPreset.rounds[currentRound - 1]) {
      const r = matchedPreset.rounds[currentRound - 1];
      return {
        isComplete: currentRound >= 3,
...
export interface CompileBrandKitParams {
  pitch: string;
  history?: Array<{
    round: number;
    question: string;
...
export async function compileBrandKit({
  ...
  groqApiKey = null,
  useMock = false
}: CompileBrandKitParams): Promise<BrandKit> {
  const matchedPreset = PRESET_PITCHES.find(
    (p) => pitch.toLowerCase().includes(p.title.toLowerCase()) || pitch.toLowerCase().includes(p.id)
  );
...
Synthesize the full Brand Kit JSON specifically tailored to this venture.`;
  ...
  try {
    const parsed = await generateStructuredJson<BrandKit>({
      systemInstruction,
      prompt: promptContent,
      apiKey,
      groqApiKey
    ...
    const error = err as Error;
    console.warn(`[GeminiService] LLM Synthesis failed, falling back to domain-aware mock:`, error.message);
    if (matchedPreset) {
      return matchedPreset.brandKit;
    }
    return generateGenericMockBrandKit(pitch, history);
