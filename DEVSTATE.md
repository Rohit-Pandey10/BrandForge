...
<<<<<<< HEAD
  - `BrandKitDashboard.jsx`: 4-tab segmented brand monograph (`Live Brand Preview`, `Brand Strategy`, `Visual Design Tokens`, `Launch Copy & Manifesto`) with embedded `"Save to Library"` action and floating export toolbar.
  - `LivePreviewTab.jsx`: Domain-adaptive website specimen styled 100% dynamically from the suggested brand color palette (`primary`, `secondary`, `accent`, `surface`, `text`), featuring active palette swatches and desktop/mobile viewport toggles.
=======
